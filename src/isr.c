//==================================================================================================
//
// File: isr.c
//
// Purpose:
//
//
//==================================================================================================


//==================================================================================================
// Include directives.
//==================================================================================================
#include "isr.h"
#include "digital.h"


//==================================================================================================
// Function definitions.
//==================================================================================================

// Enable interrupts globally.
void ISR_GlobalInterruptEnable(void)
{
    asm("SEI");
}

// Disable interrupts globally.
void ISR_GlobalInterruptDisable(void)
{
    asm("CLI");
}

// Enable a specific external interrupt.
void ISR_ExternalInterruptEnable(EXT_INT Interrupt)
{
    if      (Interrupt == EXT_INT_0) { EIMSK |= (1 << INT0); }
    else if (Interrupt == EXT_INT_1) { EIMSK |= (1 << INT1); }
    else                             { }
}

// Enable a specific external interrupt.
void ISR_ExternalInterruptDisable(EXT_INT Interrupt)
{
    if      (Interrupt == EXT_INT_0) { EIMSK &= ~(1 << INT0); }
    else if (Interrupt == EXT_INT_1) { EIMSK &= ~(1 << INT1); }
    else                             { }
}

// Initialize external interrupt.
void ISR_ExternalInterruptInit(EXT_INT Interrupt, EXT_INT_SC SenseControl)
{
    switch (SenseControl)
    {
        case EXT_INT_SC_LOW:
        {
            if      (Interrupt == EXT_INT_0) { EICRA &= (0x03); }
            else if (Interrupt == EXT_INT_1) { EICRA &= (0x03 << 2); }
            else                             { }
            break;
        }
        case EXT_INT_SC_ANY:
        {
            if      (Interrupt == EXT_INT_0) { EICRA &= (0x03); EICRA |= (1 << ISC00); }
            else if (Interrupt == EXT_INT_1) { EICRA &= (0x03 << 2); EICRA |= (1 << ISC10); }
            else                             { }
            break;
        }
        case EXT_INT_SC_FALLING:
        {
            if      (Interrupt == EXT_INT_0) { EICRA &= (0x03); EICRA |= (1 << ISC01); }
            else if (Interrupt == EXT_INT_1) { EICRA &= (0x03 << 2); EICRA |= (1 << ISC11); }
            else                             { }
            break;
        }
        case EXT_INT_SC_RISING:
        {
            if      (Interrupt == EXT_INT_0) { EICRA &= (0x03); EICRA |= ((1 << ISC01) | (1 << ISC00)); }
            else if (Interrupt == EXT_INT_1) { EICRA &= (0x03 << 2); EICRA |= ((1 << ISC11) | (1 << ISC10)); }
            else                             { }
            break;
        }
        default:
        {
            break;
        }
    }

    ISR_ExternalInterruptEnable(Interrupt);
    ISR_GlobalInterruptEnable();
}


//==================================================================================================
// Interrupt service routine for external interrupt request 0.
//==================================================================================================
ISR(INT0_vect)
{
    Digital_TogglePin(Pin3);
    Digital_TogglePin(Pin4);
    return;
}